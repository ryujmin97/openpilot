...
Worker: Claude (59차 -- 브랜치 버전관리(carrot-ryu-vN 아카이브+재생성) 정책 신설, c3-ms-dev 삭제/carrot-ryu-v1 생성. 코드 변경 없음)
Date: 2026-09-16
Repository: ryujmin97/openpilot
Code Branch: carrot-ryu (base: `9ccf1206a034c5fb5e5f35201553f9fc4e5237e5`, 55cha flip fix. 이번 세션 코드 변경 없음. 단 브랜치 구성 변경: c3-ms-dev 삭제 + carrot-ryu-v1 아카이브 신규 생성 -- 반영 스크립트 실행 대기)
Note Branch: carrot-ryu-note (base: `9f669bd81af297248d1faa30fadcb380f3c4a0db`, 58차 시점 devnotes. 이 커밋으로 devnotes 갱신)
carrot-ms 마지막 검토/동기화 커밋(메시지 기준): 904fd107b529f4636846bedcf645e102fce007b7, 58차와 동일(변경 없음).

작업:
사용자가 2절(carrot-ms 신규 커밋 개별 선별 반영) 방식의 누적 부담을 지적하며 대안을 제안: carrot-ryu 브랜치명은 디바이스 배포 대상이라 계속 고정하되, 리셋 시점마다 직전 상태를 carrot-ryu-vN으로 스냅샷 아카이브한 뒤 carrot-ms 최신 HEAD를 새 베이스로 carrot-ryu를 재생성하고, vN의 커스텀 코드(CURRENT_STATUS.md "코드 수정 현황" 번호 리스트)를 체크리스트 삼아 이식하는 방식. 논의를 통해 (a) 아카이브 생성은 히스토리 파괴 없는 단순 브랜치 생성이라 저위험, (b) carrot-ryu 재생성은 기존 히스토리를 통째로 교체하는 15절 "삭제 후 재생성" 예외 사례이므로 매번 실행 직전 사용자 승인 필수, (c) 이식은 git cherry-pick이 아니라 9절 기존 워크플로(Replace-Block/전체교체+검증)를 따르는 것으로 구체화. PROJECT_INSTRUCTIONS_carrot-ryu.md 20절로 신설, 1절 브랜치 표/원칙 문구와 18절 금지 목록도 함께 수정. 부수적으로 1절 브랜치 구성을 재확인하는 과정에서 지침에 없는 `c3-ms-dev` 브랜치를 발견, 사용자가 삭제를 지시함.

완료:
1. PROJECT_INSTRUCTIONS_carrot-ryu.md 20절(브랜치 버전 관리) 신설 + 1절 브랜치 표에 carrot-ryu-vN 아카이브 행 추가 + "두 브랜치만 존재" 문구를 "carrot-ryu-vN 아카이브 포함"으로 수정 + 18절에 관련 금지 항목 3개 추가(devnotes만 갱신, 반영 스크립트 실행 대기).
2. CURRENT_STATUS.md에 59차 항목 추가(devnotes만 갱신, 반영 스크립트 실행 대기).
3. WIP_SYNC.md에 59차 체크포인트(브랜치 버전관리 정책 신설 기록) 추가(devnotes만 갱신, 반영 스크립트 실행 대기).
4. c3-ms-dev 브랜치 삭제 + carrot-ryu-v1 아카이브 브랜치 생성 스크립트 작성(실행 대기, 코드 브랜치 구조 변경이지만 carrot-ryu 내용 자체는 변경 없음).

미완료 (다음 세션 최우선):
0. [신규, 최우선] c3-ms-dev 삭제 + carrot-ryu-v1 생성 스크립트 실행 확인(git ls-remote --heads로 재확인). 이후 20절 절차의 실제 첫 리셋(carrot-ms 최신 HEAD 기준 carrot-ryu 재생성 + carrot-ryu-v1의 코드 수정 현황 36개 항목 이식) 착수 여부/순서를 사용자와 다시 확인. 규모가 크므로(종방향/HUD/스크린샷/Google Drive/모델셀렉터 등 서로 무관한 서브시스템) 여러 세션에 걸쳐 영역 단위로 진행.
0-1. [신규] 아래 1~6번(기존 2절 개별 선별 반영 이월분, 클러스터 A/B/C 등)을 20절 리셋으로 흡수할지, 그대로 2절 절차로 마저 진행할지 사용자와 확정 필요 -- 확정 전까지는 아래 이월 항목을 그대로 유지.
