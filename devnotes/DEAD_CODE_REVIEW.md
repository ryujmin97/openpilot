...
| 신규(123차) C그룹: `cluster/cluster_renderer.py`/`cluster_scene.py`/`main.py` 완전 고아 함수 18개 + `cluster_scene.py`의 미사용 `from statistics import median` | `cluster_renderer.py`(`rectangles_overlap`/`label_rect_inside_bounds`/`render_to_file`/`_world_label_bounds`/`_draw_drive_status_box`/`_draw_follow_gap_status`/`_draw_follow_vehicle_icon`), `cluster_scene.py`(`lane_marking_segments_for_marking`/`dashed_centerline_segments`/`model_line_points_for_render`/`corner_radar_common_lateral_speed_mps`/`detected_vehicle_is_rear_corner_summary`/`radar_point_is_outside_road_edges`/`translate_vec3_x`/`translate_vehicle_box_x`/`translate_radar_marker_x`), `main.py`(`route_state_has_cutin`/`route_state_cutin_candidates`) | 18개 이름 모두 저장소 전체 참조 0곳(그룹 내부 상호호출 제외, 119차 조사와 123차 재확인 결과 동일) | 제거 완료(123차, carrot-ryu `a0f4c5a5`, 부모 `2efdd2e2`, 3개 파일 +0/-327, 실차 검증 미실시) |
| 신규(135차) `Candidate.path_in_score`/`path_out_score`(@property) + `_clamp_probability`(@staticmethod) | `radar/tools/radar_validation_replay.py` | 3개 이름 모두 저장소 전체 참조 0곳(정의 줄 외) | 코드 스크립트 준비 완료, 실행/push 대기(135차) |
...
## 123차 -- 4차 배치 C그룹 삭제 스크립트 (조사는 119차 참고)

- 대상 커밋 기준: carrot-ryu `2efdd2e2`(121차 HEAD, 이번 세션 시작 시점까지 변경 없음). 착수 전 11절(codeload tarball 재확인)을 다시 수행: 18개 함수 모두 정의 1곳 + (그룹 내부 상호호출 제외) 외부 참조 0곳을 저장소 전체 grep(`--include=*.py`, tinygrad_repo/opendbc_repo 제외)으로 재확인. `test_cluster_renderer.py`/`test_cluster_main.py` 자체가 저장소에 없고, `test_cluster_scene.py`도 대상 9개 함수를 참조하지 않음을 확인.
- 삭제 범위: `cluster_renderer.py` 7개(모듈 최상위 함수 2개 + `ClusterUiRenderer` 메서드 5개), `cluster_scene.py` 9개(전부 모듈 최상위 함수 -- `corner_radar_common_lateral_speed_mps` 삭제로 `from statistics import median` import도 미사용이 돼 함께 제거, 119차가 예고했던 미사용 import 5건 중 이 1건에 해당), `main.py` 2개(`route_state_cutin_candidates`는 `route_state_has_cutin` 안에서만 호출되던 연쇄 고아). AST(`ast.parse`)로 함수 경계(데코레이터 포함)를 정확히 산출하고, 삭제 후 남는 빈 줄 수가 원본 규칙(모듈 최상위 함수 사이 2줄, 클래스 메서드 사이 1줄)과 정확히 같아지도록 각 구간의 끝 경계에 그 빈 줄만큼을 함께 제거했다.
- 검증: 3개 파일 `py_compile` 통과, `pyflakes` 경고가 삭제 전/후 완전히 동일(`cluster_scene.py`의 기존 무관 경고 1건, 위치만 줄어든 줄 수만큼 이동). 독립적으로 받은 codeload tarball 사본에 diff를 적용해 pre/post 블롭 해시를 `git hash-object`로 대조, 별도의 완전히 새 clone에서 반영 스크립트 로직(git clone -> pre-image 블롭 해시 가드 -> base64 파일쓰기 -> post-image 블롭 해시+BOM 확인 -> `py_compile` -> commit)을 처음부터 재현해 commit diff가 3개 파일 +0/-327(cluster_renderer.py -109/cluster_scene.py -194/main.py -24)로 예상과 정확히 일치함을 확인(9절/16절). Linux 샌드박스(bash) 재현이며 Windows PowerShell 5.1에서의 실제 실행은 아니다. pytest는 conftest/컴파일 의존성 부재로 여전히 미실시.
- 상태: 코드 스크립트(`123cha_item_c_group_dead_code_carrot_ryu.ps1`) 작성, 9절 "전달 전 필수 자가검증 체크리스트" 통과(스크립트 자체가 순수 ASCII라 BOM 불필요, `core.autocrlf=false`, 임시폴더 자동삭제, `Get-PythonCmd`+EOF 공급, base64 페이로드를 전달할 .ps1 파일 자체에서 추출해 SHA 고정 원본 대비 pre/post 블롭 해시까지 재시뮬레이션 -- 9절 항목 7). v1 첫 실행은 clone 단계 네트워크 끊김(schannel early EOF)으로 push 없이 중단됐고, v2(`123cha_item_c_group_dead_code_carrot_ryu-v2.ps1`, clone `--depth 1` + 최대 3회 재시도)로 실행·push 완료를 GitHub에서 직접 재확인했다(carrot-ryu `a0f4c5a5`, 부모 `2efdd2e2`, 3개 파일 +0/-327(cluster_renderer.py -109/cluster_scene.py -194/main.py -24), 결과 blob 3개가 스크립트 `Post` 값과 일치, 삭제한 18개 이름 HEAD 전체 `git grep -w` 잔여 참조 0건). 이로써 119차 A/B/C 4차 배치 전체 종결(A `b98620e8`, B `2efdd2e2`, C `a0f4c5a5`). pytest는 이번에도 실행하지 않았다(py_compile/pyflakes/`git grep`만). **[124차] 실차 검증 완료** -- 사용자가 실제 디바이스에서 `a0f4c5a5f`까지 git pull(로그로 확인) + 재부팅 후 주행 검증, 이상 없음. 이로써 A/B/C 4차 배치 전체(115~123차)가 실차 검증까지 완료됐다.

## 135차 -- 5차 배치 (radar_validation_replay.py 고아 3개)

- 대상 커밋 기준: carrot-ryu `15f9831e`(132차 HEAD). 변경 1개 파일(radar_validation_replay.py), 순 +0/-15줄.
- 방법: 최신 HEAD tarball을 codeload로 받아 first-party 스코프(selfdrive/carrot, carrot/model_selector, controls/lib, tools/carrot_route_vault, tools/carrot_navi_lab, tools/carrot_upload_server) 함수/메서드 정의 3,588개(저장소 전체 9,588개 정의 중)를 AST로 추출, 저장소 전체(.git 제외 약 322MB)를 대상으로 심볼별 참조를 rg로 대조(11절).
- 대상: `Candidate.path_in_score`(@property, self.score만 반환), `Candidate.path_out_score`(@property, self.path_exit_score만 반환), `_clamp_probability`(@staticmethod) -- 3개 모두 정의 줄 외 참조 0건. 같은 클래스의 `candidate.score`/`candidate.path_exit_score`는 다른 곳(3680행/5287행)에서 필드로 직접 쓰이고 있어, 나중에 편의용으로 추가됐다가 실제로는 배선되지 않은 것으로 판단.
- 오탐 배제(기존 원칙 재확인 + 신규 3종): `.vendor/turing-smart-screen-python-main/` 외부 라이브러리(기존), dashcam `_ingest_carState` 등 동적 디스패치(기존), `handle_startendtag`(static.py, HTMLParser 콜백 오버라이드, 신규), `clean_baseline`/`fake_param_key_type`/`isolated_git`(`@pytest.fixture(autouse=True)`, 신규), `run_legacy_on`/`run_legacy_off`(vision_test.py, `@register_command` 데코레이터, 신규).
- 부수 발견: `controls/lib/desire_lib/maneuver_classifier.py`가 UTF-8 디코딩 실패로 1차 AST 스캔에서 누락됐으나, 원인은 죽은 코드가 아니라 파일 내 한글 주석이 CP949로 깨진 mojibake(94차 WIP.md와 동일 계열 손상)였다. `classify_maneuver_type()`은 `desire_helper.py`에서 정상 호출 중이라 삭제 대상 아님(코드 실행에는 영향 없음, 주석만 깨짐, 별도 처리는 사용자 요청 시).
- 검증: pre-image blob hash(`2502ad0a11b9c84001e1f2fefd43a41e9e665bbd`)가 GitHub HEAD(`15f9831e`) 실측치와 일치, Replace-Block 2곳 모두 앵커 1회 매치, post-image blob hash(`6691a2b70f1d36cd53fb16e2108fa0ab149af5f8`) 일치, py_compile/pyflakes 통과(경고 없음). 로컬 bare 저장소에서 반영 스크립트 로직 전체(clone -> pre-image guard -> Replace-Block 2곳 -> post-image 확인 -> py_compile -> commit/push)를 일반 체크아웃/Windows CRLF 체크아웃 재현 두 모드로 각각 끝까지 실행, 두 모드 모두 commit diff가 1 file +0/-15로 예상과 정확히 일치(9절 9번). pwsh 7.4.6 파서 구문 오류 0건(8번), 9절 체크리스트 1~10번 전항목 통과.
- 상태: 코드 스크립트(`135cha_deadcode_batch5_code_carrot_ryu.ps1`) 작성/9절 자가검증 통과, 실행/push 대기. 실차 검증: 해당 없음.
