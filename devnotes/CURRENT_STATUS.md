...
- carrot-ryu HEAD: `c9a03b54672e838a2ab9414b8d03a68271f0073c` (76차 기준, 항목 5~10 전부 push 완료. 77차에서 항목 12(`LOG_UPLOAD_TARGETS` gdrive 추가)를 이 베이스 위에 재적용했으나 반영 스크립트는 아직 실행 대기 -- push 완료되면 HEAD 갱신 필요). carrot-ryu-v1(`9ccf1206a034c5fb5e5f35201553f9fc4e5237e5`)에 원래 36개 항목 전체가 보존됨. 실기기 배포/검증은 미실시(git pull 금지 유지 중, 이식이 반 정도라도 끝나기 전까지).
...
12. LOG_UPLOAD_TARGETS "gdrive" 누락 수정(25차, commit d338afb7) -- **[77차]** 새 베이스(`c9a03b5`) 위에 byte-exact 재적용(pre-image blob hash `f41e1bb4b4`가 원본 커밋 pre-image와 완전히 일치함을 확인, anchor 1회 매치). 반영 스크립트(`77cha_item12_log_upload_targets.ps1`) 실행 대기. 실차 검증: 미실시.
