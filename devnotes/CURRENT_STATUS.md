...
- carrot-ryu HEAD: 48c2e081 (22차 params_keys.h에 CarrotGDrive* 3종 등록 반영 완료, raw.githubusercontent.com으로 직접 재확인됨)
- carrot-ryu HEAD 근처에 "token test"라는 내용 없는 빈 커밋(2c33603, 10차 위)이 있음(사용자의 git 인증 테스트로 추정, 코드/devnotes 영향 없음)
- carrot-ms 동기화 상태: 6차 세션 이후 신규 커밋(rebase) 없음 확인(7차). 8차~22차 세션에서는 동기화 재점검 없음
- 참고: carrot-wip(ajouatom/openpilot)은 계속 진행 중이나, carrot-ms가 아직 rebase하지 않아 직접 비교 대상 아님
- [22차] PROJECT_INSTRUCTIONS_carrot-ryu.md는 21차 버전(9절 .ps1 파일 전달 + BOM 필수)이 최신. 22차에서는 지침 문서 자체 변경 없음.
...
9. params_keys.h에 CarrotGDriveClientId/Secret/RefreshToken 등록(22차, commit 48c2e081) -- GitHub 반영됨. 15~20차에서 만든 Drive 연동 전체가 이 등록 누락으로 "연결" 버튼을 누르는 순간부터 항상 실패했을 버그를 수정(FINDINGS.md 2026-09-14 참고)
...
- [22차] 위 15~20차 전체가 사용하는 3개 파라미터가 params_keys.h에 미등록되어 있었던 버그를 발견/수정(핵심 발견 12 참고). 즉 15~20차 코드 자체는 문제 없었으나, 실제로는 "연결" 버튼을 누르는 순간부터 항상 실패해 한 번도 정상 동작할 수 없었던 상태였음.
- 미완료: Drive 파라미터 3종 PARAMS_REGISTRY.md 등록은 22차에서 완료. 프론트엔드 연결 UI 미이식, docs/carrot_web_upload.md 미갱신.
- 실제 Google Drive 계정으로의 업로드 테스트, 실차 검증 전부 미실시.

## 핵심 발견 11 (17~20차) -- diff(git apply) 방식의 실전 한계, 결국 예외적 보조로 격하
17차에서 처음으로 diff/git apply 방식을 실전 반영에 시도했으나, 채팅으로 전달된 patch의 컨텍스트 공백 줄이 손상되어 corrupt patch 에러로 실패함(carrot-ryu에는 영향 없이 안전하게 중단됨). 18차에서는 더 나쁜 형태로 재발함: git apply가 스크립트상 에러 없이 "적용 완료"로 보고했으나 실제로는 아무 것도 반영되지 않고, 삭제되어야 할 임시 patch 파일 3개만 carrot-ryu에 잘못 커밋됨. 19~20차에서 "파일 전체 텍스트에서 블록을 통째로 찾아 .Replace()로 치환, 치환 전 블록이 정확히 1회만 존재하는지 검증 후 치환"하는 방식으로 전환해 성공. 22차(이번 세션, params_keys.h 수정)에서도 동일 방식으로 정상 반영 확인.

## 핵심 발견 12 (22차) -- Google Drive 연동 파라미터 3종이 params_keys.h에 미등록되어 실전에서 항상 실패했을 버그
- gdrive_upload.py(15차)가 사용하는 CarrotGDriveClientId/CarrotGDriveClientSecret/CarrotGDriveRefreshToken이 openpilot/common/params_keys.h에 전혀 등록되어 있지 않았음. openpilot Params.get()/.put()은 미등록 키에 UnknownKeyName 예외를 던지므로, Drive "연결" 버튼을 누르는 첫 단계(client_id 저장)부터 항상 HTTP 500으로 실패하는 상태였음.
- 22차에서 params_keys.h에 3줄 추가(commit 48c2e081)로 수정. 실제 Drive 연결 테스트는 여전히 미실시.
- 상세: FINDINGS.md 2026-09-14 항목 참고.

- 미확인: carrot-ms 모델 셀렉터 코드 미분석
- 다음 작업: 실제 Drive 연결 테스트(사용자 액션 필요), 프론트엔드 UI 이식, docs 갱신, 코드 수정 9건 전부 실주행 재검증, 모델 셀렉터 코드 분석
