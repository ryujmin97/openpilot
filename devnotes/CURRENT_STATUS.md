...
- **[140차, devnotes만 · 코드 변경 없음]** 139차 반영 스크립트가 실제 Windows PowerShell 5.1에서 `git ... 2>&1 | Write-Host` 패턴 때문에 `git clone` 단계에서 `NativeCommandError`로 즉시 중단되는 문제 발생(정상 진행 메시지인 stderr를 오류로 오인, `$ErrorActionPreference="Stop"`으로 중단) -- `Invoke-Git` 헬퍼(스트림 비병합 + `$LASTEXITCODE` 확인)로 두 스크립트(코드/devnotes) 모두 수정 후 재검증, 사용자가 수정본 실행 완료. GitHub raw 조회로 carrot-ryu 4개 파일 import 삭제 + carrot-ryu-note WIP/HANDOFF 139차 기록 반영을 직접 재확인(16절) -- 139차(코드+devnotes) 완료 확정. FINDINGS.md에 핵심 발견 53 신규 등록(원인/수정/일반화, `Invoke-Git` 패턴을 향후 반영 스크립트 기본값으로 채택). 실차 검증: 해당 없음. 상세: WIP.md 140차 참고.
- **[139차, 코드 1건 · 미사용 import 4건 삭제 배치 · 실행/push 대기]** 세션 시작 138차 devnotes push 재확인(`66514fa`) + carrot-ms 2절 정기 점검(신규 커밋 없음, `3756e6d5`) 이후, 사용자 요청으로 `controls/lib` 스코프 추가 데드코드 스캔 진행 -- 함수/메서드 단위 신규 고아 없음(제로-참조 후보 14개 전부 119차 기존 제외 패턴과 일치), 미사용 import 4건 신규 발견(`drive_helpers.py`/`lateral_planner.py`/`latcontrol_angle.py`/`desire_lib/maneuver_classifier.py`) 후 사용자 승인. 반영 스크립트는 9절 체크리스트 전항목(BOM 누락 발견/수정 포함, `core.autocrlf`/finally/anchor 재확인/pwsh 파서 0 errors/`-C`/pre-post blob hash guard 신규 추가/로컬 bare 저장소 일반+Windows CRLF 재현 두 모드 byte-exact 일치, 실제 `.gitignore` 반영해 `.pyc` 오혼입 없음 확인) 통과. 실행/push 대기. 실차 검증: 해당 없음. 상세: WIP.md 139차 참고.
- **[138차, devnotes만 · 코드 변경 없음]** 137차 devnotes push(commit `f476d53d`) 확인을 받아 HANDOFF.md(137차) 미완료 2번(136차 v1 py_compile 중복 호출 버그를 FINDINGS.md 핵심 발견으로 정식 등록)을 처리 -- FINDINGS.md에 핵심 발견 52 신규(원인/수정안/일반화 기록), 반영 스크립트는 `devnotes/toolkit/replace_block_template.ps1`의 `Invoke-ReplaceBlock-CrlfNative` 재사용, 9절 체크리스트 전항목(BOM/`core.autocrlf`/finally 정리/WriteAllText 무BOM/anchor 결과 재확인/pwsh 파서 0 errors/로컬 bare 저장소 일반+Windows CRLF 재현 두 모드 byte-exact 일치/`-C` 사용) 통과. 실행/push 대기. 코드 변경 없음.
- **[137차, devnotes만 · 코드 변경 없음]** 사용자가 제공한 실기기 도구 탭 업데이트 로그 스크린샷으로 123차(C그룹 dead code, `a0f4c5a5`)와 132/135/136차(미사용 import 정리/데드코드 5차 배치/mojibake 수정, `~41e4c056`) 두 차례의 `git pull`+`reboot` 배포를 확인, 사용자가 실주행 "이상없음"을 확인함(12절: 6개 커밋 모두 비-실행로직 변경(데드코드/import/주석)이라 총괄 수준 무회귀 확인 -- 97~114차 GATE_M/MAP_TURN_GUIDE_FACTOR 등 종방향 로직 개별 검증은 아님, 여전히 이월). 코드 변경 없음.
- **[136차, 코드 스크립트 검증 완료 · 실행/push 대기 · 135차 push 확인]** 세션 시작 체크포인트(`git ls-remote`)로 carrot-ryu HEAD가 이미 `4faf9072`(135차 데드코드 5차 배치, `radar_validation_replay.py`)로 push 완료돼 있음을 확인 -- HANDOFF.md(135차)의 코드 스크립트 "실행/push 대기" 표기와 달랐던 괴리(16절/핵심 발견 27·38과 동일 패턴), commit patch로 변경 파일 1개(-15줄)가 135차 기록과 정확히 일치함을 재확인. 이어서 사용자가 업로드한 반영 스크립트(`136cha_mojibake_fix_code_carrot_ryu.ps1`, 135차 미완료 5번 -- `desire_lib/maneuver_classifier.py` 한글 주석 CP949 mojibake -- 대응)를 핵심 발견 26 원칙(타 세션 스크립트도 실행 전 재검증)에 따라 독립 재검증: pre-image blob hash(`0021af97d7`)가 현재 HEAD 실측치와 일치, CP949->UTF-8 디코딩 독립 재현으로 post-image blob hash(`b552e165`) 일치 + 변경 라인이 정확히 2곳(23·27번째 줄, 둘 다 `#` 주석)뿐임을 확인, `py_compile` 통과, BOM 미포함 확인, pwsh 7.4.6 파서 구문 오류 0건, 로컬 bare 저장소(대상 파일 최소 시뮬레이션) 일반/Windows CRLF 재현 두 모드 모두 commit diff "+2/-2"로 byte-exact 일치, 저장소 상태 조회 git 명령 전부 `-C` 사용 확인(10절). 실행/push 대기. 실차 검증: 해당 없음(주석 텍스트만 수정). **[136차 계속]** v1 스크립트가 실제로는 py_compile 중복 호출 버그(Push-Location 이전 상대경로 호출)로 push 전에 실패했음을 확인, 중복 호출 제거+절대경로 사용으로 고친 v2로 재실행해 commit `41e4c056d8db2ceb38fe93c114ca5a3be3d8de8f`로 최종 push 완료(GitHub `.patch`+blob hash byte-exact 재확인).
- **[135차, 코드 1건 · 데드코드 5차 배치 스크립트 준비 완료 · 실행/push 대기]** 새 데드코드 후보 재스캔(11절, carrot-ryu 최신 HEAD `15f9831e` tarball, first-party 스코프 정의 3,588개 AST + 저장소 전체 rg 대조) 결과 `radar_validation_replay.py`의 `Candidate.path_in_score`/`path_out_score`(@property) + `_clamp_probability`(@staticmethod) 3개만 참조 0건으로 확정(사용자 승인). 오탐 배제 유형에 HTMLParser 콜백 오버라이드/`@pytest.fixture(autouse=True)`/`@register_command` 3종 신규 확인. 부수 발견: `controls/lib/desire_lib/maneuver_classifier.py`가 한글 주석 CP949 mojibake로 AST 파싱 실패해 1차 스캔에서 누락됐으나 `classify_maneuver_type()`은 `desire_helper.py`에서 정상 호출 중이라 삭제 대상 아님(주석만 깨짐, 코드 실행 무관, 별도 처리 보류). 코드 반영 스크립트(`135cha_deadcode_batch5_code_carrot_ryu.ps1`) 작성, 9절 자가검증 체크리스트 1~10번 전항목 통과(pre/post blob hash 일치, py_compile 통과, pwsh 파서 0 errors, 로컬 bare 저장소 일반/Windows CRLF 재현 dry-run 2모드 모두 byte-exact 성공, commit diff 1 file +0/-15). 실행/push 대기. 실차 검증: 해당 없음. 상세: WIP.md 135차, DEAD_CODE_REVIEW.md 135차 참고.
- **[134차, devnotes만 · 코드 변경 없음]** 133차 스크립트 자체의 pre-image guard가 `git hash-object`를 `-C` 없이 절대경로로 호출해, PowerShell CWD(`C:\WINDOWS\system32`)가 clone 폴더 밖일 때 로컬 `core.autocrlf=false` override를 무시하고 전역 설정(Windows Git 기본값 `core.autocrlf=true`)으로 FINDINGS.md 해시를 잘못 계산 -- 내용은 실제로 같은데 "base drifted"로 오탐돼 1차 실행이 안전 중단됐던 근본 원인을 확정(132차 핵심 발견 50과는 별개 메커니즘). 두 지점을 `-C $Tmp`로 수정해 사용자가 재실행, carrot-ryu-note `49146b3` 성공 push까지 GitHub 직접 재조회로 확인. FINDINGS.md 핵심 발견 51 신규, PROJECT_INSTRUCTIONS_carrot-ryu.md 9절 체크리스트 10번 신설(저장소 상태 조회 git 명령은 `-C` 필수). 상세: WIP.md 134차 참고.
- **[133차, 지침/toolkit만 · 코드 변경 없음]** HANDOFF.md(132차)의 "미완료 1번"(두 반영 스크립트 실행/push 확인)이 채팅에 붙여넣어진 사본과 달리 GitHub 재조회 결과 **이미 완료**돼 있었음을 확인/정정(carrot-ryu `15f9831e`, carrot-ryu-note `48f2ff17` 모두 확인). anchor 0회 실패 원인은 핵심 발견 44/46/48과 동일한 `.gitattributes` `* text=auto`로 인한 Windows CRLF 체크아웃이며, 121차가 이미 반증된 "core.autocrlf=false로 구조적으로 차단됨" 결론을 재확인 없이 재채택했던 것이 근본 원인(FINDINGS.md 핵심 발견 50 신규). 재발 구조 자체를 막기 위해 `devnotes/toolkit/replace_block_template.ps1`(재사용 가능한 `Invoke-ReplaceBlock`/`Invoke-ReplaceBlock-CrlfNative` 공통 헬퍼, 기능 테스트 완료) 신규 등록 + `PROJECT_INSTRUCTIONS_carrot-ryu.md` 9절/18절 강화. 상세: WIP.md 133차, FINDINGS.md 핵심 발견 50 참고.
- **[132차, 코드 1건 · 미사용 import 정리]** 131차 이월분(남은 17건) 처리 완료 -- carrot_serv.py 10건 + carrot_man.py 4건(+파생 발견 urllib.request 1건, 총 5건) + dashcam upload.py 1건(upload_message_lines) = 15건 삭제, 2건(upload.py의 upload_share_text, radar/tools/radar_lead_simulator.py의 radar_validation_replay 와일드카드)은 저장소 전체 grep으로 실제 외부 소비자(upload_jobs.py의 upload.upload_share_text(), test_radar_lead_simulator.py 등의 재-export 이름 직접 import)를 확인해 오탐으로 판단, 보존(10절 최소 변경 원칙). py_compile + pyflakes 재스캔 + 로컬 bare 저장소 dry-run(9절 체크리스트 9번, blob hash byte-exact 일치)으로 검증. 상세: WIP.md 132차 참고.
- **[131차, 코드 1건 · 고아 파일 삭제]** 세션 시작 시 `git ls-remote`로 carrot-ryu HEAD가 이미 `3759a300`(고아 파일 `cluster_ui.py` 삭제, 미사용 import 24건 해결)까지 나가 있는데 devnotes는 130차에 멈춰 있음을 발견(16절) -- 채팅에 붙여넣어진 주장을 신뢰하지 않고 GitHub 직접 재조회(`git show --numstat`/raw 404/pyflakes 재현)로 독립 검증(3절, 11절). 기존 `46→22` 표기의 스코프가 devnotes에 기록돼 있지 않아 재구성 불가했던 대신, `openpilot/selfdrive/carrot/` 스코프로 pyflakes를 재실행해 남은 미사용 import 17건(`carrot_serv.py`/`carrot_man.py`/`dashcam/upload.py`/`radar_lead_simulator.py`)을 새 기준선으로 확정, 132차로 이월. 상세: WIP.md 131차 참고.
- **[130차, devnotes 정정 + 정기 점검 · 코드 변경 없음]** carrot-ms 2절 정기 점검 -- `git ls-remote`로 happymaj11r/openpilot HEAD가 `3756e6d5`로 129차 체크포인트와 동일함을 확인(신규 커밋 없음). 이어서 아래 97~114차 catch-up 항목이 실제로는 이미 124차에서 작성 완료돼 있음을 재확인 -- 125~129차 HANDOFF.md가 "여러 세션째 이월 중(122차부터)"으로 이 항목을 계속 잘못 복사해온 stale carryover였음을 발견(16절, 핵심 발견 18/27/38과 동일 계열). HANDOFF.md의 해당 미완료 항목을 제거해 바로잡았다. 코드 변경 없음. 상세: WIP.md 130차 참고.
- **[129차, devnotes만 · 코드 변경 없음]** 128차에서 이월된 carrot-ms 후보 2건(9f8619b1, 3756e6d5) 최종 판단 -- 9f8619b1(레이더 CAN 전처리를 card에서 core4 신규 프로세스 radarcan으로 분리)을 실제 `git clone`+`git apply --check`로 상세 대조한 결과 핵심 코드 16개 파일 전부 충돌 없이 적용 가능함(py_compile 통과, dangling reference 0건)을 확인했으나, (1) 이 리팩터의 동기인 EV9/Ioniq5 card-camerad core6 경합 증상을 DH2015에서 관찰한 적 없음, (2) 원문서가 "C3/C4 device timing improvements are NOT yet validated"라고 명시, (3) 새 IPC(core4 radarcan) 타임아웃 등 검증 안 된 실패 지점이 리드 감지 경로에 추가됨(97~114차 종방향 게이팅 실차 미검증과 겹침), (4) DH2015는 RadarInterface 연산이 가벼워(EnableRadarTracks=0/EnableCornerRadar=0) 이득도 제한적 -- 4가지 근거로 반영 보류(제외 확정, 사용자 승인). 3756e6d5는 9f8619b1(radarcan.py)에 구조적으로 종속돼 자동 제외. 이로써 128차에서 발견한 carrot-ms 신규 10건 전부 분류 종결(제외 8+2=10건, 반영 0건). 코드 변경 없음, devnotes만 갱신. 상세: WIP_SYNC.md 129차 체크포인트, WIP.md 129차 참고.
- **[128차, devnotes만 · 코드 변경 없음]** carrot-ms 동기화 점검(2절) 착수 -- 마지막 체크포인트(116차, `4bb4b510`) 대비 rebase 발생(hash 체인 불일치, `git merge-base --is-ancestor`로 비조상관계 확인) -> 커밋 메시지 기준 대조(400/410건 로그 diff, OLD-only 0건)로 신규 10건(전부 2026-09-21, ajouatom, cherry-picked) 확인. 개별 diff 직접 대조 결과 CAN FD 전용 4건(`e6baf4f9`/`eebecda0`/`fb808e4e`/`3225e8c6`, DH2015 LEGACY 경로 미실행 확인) + 테스트/무관 브랜드 1건(`9513408e`, KIA_EV6 리네임+replay tooling) + VW MEB 전용 1건(`7ff3a457`, Hyundai 분기 사실상 무영향) + 클러스터 기능 2건(`c42437d9`/`e2fe3e72`, 사용자 미사용·계획없음 확인) 총 7건 제외 확정. 나머지 2건은 다음 세션 후보로 이월: `9f8619b1`(레이더 CAN 전처리를 card에서 분리하는 아키텍처 리팩터, 우리 radar_motion 커스텀과 충돌 대조 필요) + `3756e6d5`(camerad/UI core5 배치 트라이얼, 회귀테스트가 `radarcan.py`(core4)를 전제해 `9f8619b1` 반영 이후에만 성립함을 코드로 확인 -- 독립 반영 불가, 아이오닉5 C4 전용 증상 대응이라 DH2015 재현 근거도 없음). 상세는 WIP_SYNC.md 128차 체크포인트 참고.
