...
- 베이스 브랜치: carrot-ms (happymaj11r/openpilot). ryujmin97/openpilot에는 더 이상
  ...
  직접 조회)
- carrot-ryu 최신 commit: 2dbe492 (8차 이후 route 감속 근본수정 1건 반영,
  carrot-ms(0201519)와 코드 차이 발생 시작 - 10절 원칙에 따라 차이 내역 추적 중)
- carrot-ms 동기화 상태: 6차 세션 이후 carrot-ms에 신규 커밋(rebase) 없음 확인
  (7차, 2026-09-13). 8~9차 세션에서는 동기화 재점검 없음
- 참고: carrot-wip(ajouatom/openpilot) HEAD는 bb0e18bb8c09422fcd50dcf25c17e0d5c75072b1로
  ...
  (2절 원칙)
- 상태: 코드 변경 1건 반영됨 (route 감속 오검출 근본수정), 실차 재검증 대기 중
...
- 핵심 발견 6 후속 (9차): 위 오검출에 대해 3-샘플 median 스파이크 제거 필터를
  carrot_navi_route()에 추가함(carrot-ryu commit 2dbe492). 정적/합성 검증만 완료,
  실주행 재검증 대기 중 (다음 세션 최우선)
- ⚠ 미확인: carrot-ms가 추가한 "모델 셀렉터" 관련 코드(carrot/model_selector 등)는
  아직 전혀 분석하지 않음
- 다음 작업: ①route 감속 수정 실주행 재검증(최우선), ②모델 셀렉터 코드 분석,
  ③콤마 디바이스 실차 장착 → 추가 route 로그 수집, ④TurnSpeedControlMode/
