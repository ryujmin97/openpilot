...
- carrot-ryu HEAD: 5f5e49d0 (27차, 우측하단 경로안내 박스 475x495 확대 + route= 분리 표시 + 도착 2줄 표기, 반영 후 GitHub 재조회로 확인 완료)
- carrot-ryu HEAD 근처에 "token test"라는 내용 없는 빈 커밋(2c33603, 10차 위)이 있음(사용자의 git 인증 테스트로 추정, 코드/devnotes 영향 없음)
- carrot-ms 동기화 상태: 6차 세션 이후 신규 커밋(rebase) 없음 확인(7차). 8차~27차 세션에서는 동기화 재점검 없음
- 참고: carrot-wip(ajouatom/openpilot)은 계속 진행 중이나, carrot-ms가 아직 rebase하지 않아 직접 비교 대상 아님
- PROJECT_INSTRUCTIONS_carrot-ryu.md는 22차 버전(4절 0번 단계 보고 의무화)이 최신. 27차에서도 이 문서를 최우선 조회함.
...
13. 우측하단 경로안내 박스 475x495 확대 + route=숫자 디버그 분리 표시 + 도착정보 2줄 표기(27차, commit 5f5e49d0) -- GitHub 반영됨, 반영 후 GitHub 재조회로 확인 완료
...
- 프론트엔드 연결 UI(23차): web_settings에 Drive 연결 버튼/Client ID·Secret 입력/인증 코드 표시 UI 추가(commit 272834b). 실기기에서 드롭다운은 바뀌었으나 입력란이 안 보이는 문제 계속 진행 중(26차, 핵심 발견 16).
...
17~18차에서 diff/git apply가 두 차례 실전 문제(corrupt patch, 조용한 미반영)를 일으켜 19~20차에 문자열 블록 치환(Replace-Block, 치환 전 블록 정확히 1회 매치 검증)으로 전환. 22, 24, 25, 27차에서도 동일 방식으로 정상 반영/검증 확인.
...
23차에서 프론트엔드 드롭다운에만 value="gdrive" 옵션을 추가하고 server/services/web_settings.py의 LOG_UPLOAD_TARGETS = {"carrot", "toss"}는 갱신하지 않아, "gdrive" 선택이 백엔드에서 유효하지 않은 enum 값으로 취급됨. 25차에서 수정 완료(commit d338afb7). 다만 이 수정 이후에도(26차 실기기 검증) 입력란 미노출 현상은 그대로 재현됨 -> LOG_UPLOAD_TARGETS는 원인이 아니었거나, 원인 중 일부만 해결한 것으로 보임(핵심 발견 16).
...
반영 스크립트가 clone 직후 파일을 LF 기준 문자열로 치환하려 했으나, 사용자 Windows 환경의 Git이 checkout 시 LF를 CRLF로 자동 변환(core.autocrlf)해 첫 번째 치환 블록부터 0회 매치로 실패함(commit/push 이전 단계라 carrot-ryu에는 영향 없이 안전하게 중단됨, 임시 폴더도 정상 삭제 확인). git clone에 --config core.autocrlf=false 옵션을 추가하고, 파일을 읽은 직후 CRLF->LF로 정규화하는 안전장치를 코드 반영 스크립트에 넣어 해결. 앞으로 이 저장소 대상 코드 반영 스크립트에는 기본적으로 이 옵션을 포함할 필요가 있음(9절 규칙화 여부는 다음 세션에서 논의 예정, 19절 절차 필요).

- 미확인: carrot-ms 모델 셀렉터 코드 미분석
- 다음 작업: (최우선) 실기기 터미널로 배포된 tools.js 내용 확인해 번들 최신 여부 검증, 화면녹화 전송 다이얼로그 "당근서버" 라벨 하드코딩 조사, test_web_upload.py 실제 실행해 낡은 테스트 범위 확정, 데드코드 3개 삭제 + 대응 테스트 정리, 실제 Drive 연결 테스트(사용자 액션 필요), docs 갱신, 코드 수정 13건 전부 실주행 재검증, 모델 셀렉터 코드 분석, 9절에 core.autocrlf=false 기본 포함 규칙 추가 여부 논의
