...
- carrot-ryu HEAD: 39차 커밋 반영 대기(이 devnotes 작성 시점에는 아직 스크립트 미실행, HEAD는 여전히 37차 `c01d9ec`) -- 화면녹화 탭 사진 업로드 UI 신규 구현 + 경로안내 박스 상하 여백 통일. 다음 세션에서 GitHub API로 실제 반영 여부와 정확한 해시 재확인 필요. 38차는 코드 변경 없이 devnotes만 갱신함(37차 커밋 그대로였음).
...
22. 화면녹화 탭 사진 업로드 UI 신규 구현(체크박스/전체선택/다운로드/전송) + 경로안내 박스 상하 여백 통일(content_shift_y)(39차, 커밋 해시는 스크립트 실행 로그의 git push 출력 참고, 이 devnotes 작성 시점에는 아직 스크립트 미실행) -- 정적 검증(py_compile/node --check/build/npm test 737/737) 및 독립 재검증 전부 통과, 실기기 검증 대기.
...
- 다음 작업: 39차 사진 업로드 UI/경로안내 박스 여백 실기기 검증, 화면녹화 탭 "영상" 업로드 UI 실기기 검증(녹화본 확보 후), 37차 락 수정 동시성 재현 검증(의도적으로 동시에 두 업로드 시도), 34차 도로명-신호과속 같은 줄 배치 확인(신호과속 구간에서), 28~30차 레이아웃 정밀 재검증, 실기기 터미널로 배포된 tools.js 내용 확인해 번들 최신 여부 검증, test_web_upload.py 실제 실행해 낡은 테스트 범위 확정, 데드코드 3개 삭제 + 대응 테스트 정리, docs 갱신, 코드 수정 22건 전부 실주행 재검증, 모델 셀렉터 코드 분석
