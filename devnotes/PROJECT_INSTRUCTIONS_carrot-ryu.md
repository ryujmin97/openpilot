...
- 기록 전용 브랜치: carrot-ryu-note (devnotes 보관 전용, 코드는 두지 않음. 코드 히스토리와 분리된 orphan 브랜치)
...
- 이 브랜치 안에 다음 파일들을 둔다.
  ...
  - `devnotes/PROJECT_INSTRUCTIONS_carrot-ryu.md` - 이 문서 자체 (19절 참고, Claude Project instructions에는 이 파일을 가리키는 짧은 포인터만 둔다)
...
## 3. 가장 중요한 원칙
────────────────────────────────
- GitHub이 항상 Source of Truth이다. Claude의 이전 대화 기억보다 GitHub의 현재 상태를 우선한다.
- carrot-wip = 최종 원본(간접 참고, ajouatom/openpilot에 존재), carrot-ms = carrot-ryu의 직접 베이스(carrot-wip+모델셀렉터, happymaj11r/openpilot에 존재, 매번 재생성됨), carrot-ryu = 내 코드, carrot-ryu-note = 작업 기억(devnotes). 이 네 가지의 역할을 항상 구분하고, 서로의 내용이 섞이지 않게 한다. carrot-ryu/carrot-ryu-note만 ryujmin97/openpilot에 존재한다.

────────────────────────────────
## 4. 새 세션 시작 시 확인 순서 (지연 로딩, 무료 사용량 절약)
────────────────────────────────
전체 파일을 다 읽지 않는다. 필요한 만큼만, 아래 순서로 확인한다.
0. (Project instructions의 포인터를 따라) 이 문서(`devnotes/PROJECT_INSTRUCTIONS_carrot-ryu.md`) 최신 버전을 먼저 확인
1. carrot-ryu-note 브랜치의 `devnotes/HANDOFF.md` - 가장 먼저 확인 (직전 세션 인수인계)
...
- Claude가 작업을 끝냈어도, 사용자가 실제로 스크립트를 실행해 GitHub에 push하기 전까지는 "다음 세션에 반영된 것"으로 간주하지 않는다.
...
────────────────────────────────
## 9. 작업 결과물 전달 규칙 (매번 필수) [9차 세션 개정 — 코드 파일에 diff/patch 옵션 추가]
────────────────────────────────
- **기본 방식: PowerShell(PC) / Termux(폰) 스크립트.** Claude는 다음 구조의 스크립트를 만들어 전달한다.
  ...
  3. 파일 반영 — 아래 유형을 구분해서 처리:
     ...
     - **교체형(요약 파일)**: `CURRENT_STATUS.md`, `HANDOFF.md`, `PROJECT_INSTRUCTIONS_carrot-ryu.md` 등 "항상 최신 1개만 유지"하는 파일은 전체 내용을 스크립트에 담아 통째로 덮어쓴다.
...
- 한글/비ASCII 문자는 스크립트 안 문자열에 그대로 넣어도 되며, PowerShell에서는 `Set-Content -Encoding UTF8`을 사용해 인코딩 깨짐을 방지한다.
...
- 스크립트 실행 결과(특히 `git apply`/`git commit`/`git push` 출력)를 사용자가 그대로 전달해주면, Claude가 그 로그를 보고 정상 반영 여부를 확인한다. 필요하면 Claude가 GitHub API로 반영 결과를 재확인한다.
...
- **[10차 세션 추가]** Project instructions에 지침 문서 전체가 아니라 짧은 포인터만 있는데, 그 포인터가 가리키는 `devnotes/PROJECT_INSTRUCTIONS_carrot-ryu.md`를 실제로 읽지 못했거나(파일 없음/조회 실패) 내용이 이 요약과 크게 다른 경우 — 임의로 예전 기억 기준으로 진행하지 않고 사용자에게 먼저 알린다
...
- 다음 세션은 새 대화로 시작하며, Claude Project instructions의 포인터를 통해 이 문서(devnotes/PROJECT_INSTRUCTIONS_carrot-ryu.md)와 carrot-ryu-note 브랜치의 `devnotes/HANDOFF.md`부터 확인하고 이어간다.
...
- **[9차 세션 추가]** diff/patch가 `git apply`에 실패했는데 강제로 적용하거나(`--3way`/`--reject` 등으로 임의 우회 포함) 무시하고 다음 단계(commit/push)를 진행하는 것 — 실패 시 반드시 중단하고 사용자·Claude에게 보고한다.
- **[10차 세션 추가]** Claude Project instructions에 이 문서 전체를 다시 붙여넣어 중복 관리하는 것 — Project instructions에는 짧은 포인터만 두고, 실제 지침 갱신은 항상 이 GitHub 파일 쪽에서만 한다.
...
- 이 문서는 carrot-ryu-note 브랜치의 `devnotes/PROJECT_INSTRUCTIONS_carrot-ryu.md`에 보관하는 것을 표준으로 한다(10차 세션부터 실제 적용). Claude Project instructions(여러 계정 공통)에는 이 파일을 가리키는 짧은 포인터만 두어, 계정과 무관하게 항상 GitHub 상의 최신 지침을 기준으로 작업하도록 한다.
...
→ Claude Project instructions에는 짧은 포인터만 두고, 이 문서(GitHub 상의 PROJECT_INSTRUCTIONS_carrot-ryu.md)가 항상 실제 기준(Source of Truth)이다.
...
- [10차 세션] 이 문서 전체를 Claude Project instructions에 직접 넣지 않고, carrot-ryu-note 브랜치의 `devnotes/PROJECT_INSTRUCTIONS_carrot-ryu.md`에 보관한 뒤 Project instructions에는 이 파일을 가리키는 짧은 포인터만 두는 방식으로 전환. 사유: Project instructions에 문서 전체를 넣으면 매 대화 시작 시 토큰이 소모되고, 계정마다 따로 관리해야 해서 최신화가 번거로움. 포인터 방식으로 바꾸면 세션 시작 시 1회 조회로 최신 지침을 확인할 수 있고, 여러 계정에서 동일한 최신 지침을 공유할 수 있음. 1, 4, 16, 17, 18, 19절 및 최종 목표 요약을 이에 맞춰 정리하고, 이 문서 자체를 carrot-ryu-note 브랜치에 최초로 커밋함.
