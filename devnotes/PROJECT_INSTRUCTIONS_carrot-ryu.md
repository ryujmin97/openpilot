...
- 매번 다음 내용을 함께 전달한다:
  1. 실행할 스크립트 **파일**(다운로드) + 그 파일을 실행하는 **명령어**(복사해서 터미널에 붙여넣을 수 있는 형태). [21차] 스크립트 전체 텍스트를 채팅에 다시 붙여넣지 않는다 — 파일 전달로 충분하며, 중복 전달은 무료 사용량 낭비다. **[26차 세션 추가]** 기본 PowerShell 실행 정책(`Restricted`)에서는 `.ps1` 실행 자체가 `UnauthorizedAccess` 오류로 막히는 경우가 많으므로(24차, 26차에서 실제 발생), 실행 명령은 기본적으로 `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass`를 앞에 붙인 2줄 형태로 안내한다(`-Scope Process`이므로 그 PowerShell 창에만 적용되고 창을 닫으면 자동 복귀 — 시스템 전체 정책은 건드리지 않음).
