...
0. **[12차 세션 추가, 22차 세션 강화 — 반드시 조회 후 결과를 사용자에게 먼저 보고]** `carrot-ryu-note` 브랜치의 `PROJECT_INSTRUCTIONS_carrot-ryu.md` — 이 지침 문서 자체를 raw.githubusercontent.com으로 **세션의 첫 도구 호출로서** 가장 먼저 조회한다. 사용자가 채팅에 별도로 붙여넣지 않았어도 이 파일을 기준으로 진행하며, 붙여넣은 버전과 GitHub 버전이 다르면 GitHub 버전(더 최신)을 우선한다(3절 원칙).
   - **[22차 세션 추가, 필수]** 이 조회를 실제로 수행했다면, 조회해서 확인한 "최신 회차 번호"(변경 이력의 마지막 항목 기준)를 세션의 첫 응답 안에서 사용자에게 짧게 언급한다(예: "지침 문서 22차 버전 확인함"). 이 언급이 없다면 0번 단계를 건너뛰고 시스템 프롬프트에 남아있는 구버전 지침(이 문서가 만들어지기 이전에 채팅 시스템에 고정 저장된 오래된 사본)으로 작업을 시작했다는 뜻이므로, 사용자는 그 즉시 작업을 중단시키고 0번 단계부터 다시 하도록 지적한다. **22차 세션에서 이미 문서화되어 있던 [21차] UTF-8 BOM 규칙(9절, 18절)을 이 절차 누락으로 인해 다시 위반해 동일한 인코딩 오류를 반복 재현한 사례가 실제로 있었다** — 즉 "같은 버그가 매 세션 반복"되는 근본 원인은 규칙이 없어서가 아니라, 그 세션이 규칙이 적힌 문서 자체를 조회하지 않고 시작했기 때문인 경우가 있다.
...
- **기본 방식: PowerShell(PC) / Termux(폰) 스크립트를 파일로 생성해 전달.** [21차 세션 변경] 이전에는 스크립트 전체를 채팅에 그대로 붙여넣는 방식을 기본으로 했으나, 파일이 크면(특히 PROJECT_INSTRUCTIONS_carrot-ryu.md 같은 교체형 대용량 파일) 채팅 붙여넣기가 무료 사용량(토큰)을 훨씬 더 많이 쓴다는 것이 확인되어, Claude의 파일 생성 도구로 `.ps1` 파일을 만들어 전달하고 실행 명령만 채팅에 별도로 안내하는 방식을 기본으로 전환한다. 스크립트 자체의 구조는 다음과 같다.
  ...
  2. 시스템 임시 폴더 하위에 무작위 이름으로 폴더를 만들고 해당 브랜치를 `git clone --branch <branch> --single-branch --config core.autocrlf=false`. **[27차 세션 추가, 필수]** Windows Git의 기본 `core.autocrlf=true` 설정 때문에 clone 시점에 저장소의 LF(`
`) 줄바꿈이 로컬에서 CRLF(`
`)로 자동 변환되면, Claude가 LF 기준으로 만든 문자열 블록 치환(Replace-Block, 20차)의 "변경 전 블록"이 실제 파일 내용과 바이트 단위로 달라져 매치에 실패한다(0회 매치로 안전하게 중단됨, 27차에서 실제 재현). `--config core.autocrlf=false`를 clone 시 기본으로 포함해 이 문제를 원천 차단한다.
  3. 파일 반영 — 아래 유형을 구분해서 처리:
     ...
     - **코드 파일 — [20차 세션 기본 방식으로 확정] 여러 군데 부분 수정, 함수 단위 교체 등 변경 범위가 파일 크기 대비 작은 경우**: 변경 전/변경 후 블록을 각각 통째로 문자열(here-string)로 담아, 스크립트가 파일 전체 텍스트에서 "변경 전 블록"이 **정확히 1회만** 존재하는지 확인한 뒤 `.Replace()`로 통째로 교체한다(`Replace-Block` 패턴). 1회가 아니면(0회 또는 2회 이상) 그 자리에서 중단하고 아무것도 건드리지 않은 채 사용자에게 보고한다 — 강제 진행 금지(15절 원칙과 동일). Claude는 이 블록을 만들기 직전 해당 파일의 GitHub 최신 내용을 다시 조회해 그 위에서 블록을 구성한다(6절과 연계). **[27차 세션 추가]** 위 9절 2번의 `core.autocrlf=false` clone 옵션이 기본 방어선이며, 추가 안전장치로 매칭 직전 파일 내용과 "변경 전 블록" 모두 `
` → `
`로 정규화한 뒤 비교/치환하는 이중 방어를 권장한다.
...
- 매번 다음 내용을 함께 전달한다:
  1. 실행할 스크립트 **파일**(다운로드) + 그 파일을 실행하는 **명령어**(복사해서 터미널에 붙여넣을 수 있는 형태). [21차] 스크립트 전체 텍스트를 채팅에 다시 붙여넣지 않는다 — 파일 전달로 충분하며, 중복 전달은 무료 사용량 낭비다. **[26차 세션 추가]** 기본 PowerShell 실행 정책(`Restricted`)에서는 `.ps1` 실행 자체가 `UnauthorizedAccess` 오류로 막히는 경우가 많으므로(24차, 26차에서 실제 발생), 실행 명령은 기본적으로 `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force`를 앞에 붙인 2줄 형태로 안내한다(`-Scope Process`이므로 그 PowerShell 창에만 적용되고 창을 닫으면 자동 복귀 — 시스템 전체 정책은 건드리지 않음. **[27차 세션 추가]** `-Force`를 붙여 "실행 정책을 변경하시겠습니까?"류의 Y/A/N/L/S 확인 프롬프트 자체가 뜨지 않도록 한다 — 26차 세션에서 이 프롬프트가 실제로 발생함).
...
- **[26차 세션 추가] 스크립트 실행 후 뒷정리.** 전달한 `.ps1` 파일은 사용자 PC의 다운로드 폴더(`$HOME\Downloads\` — PC마다 사용자 계정명이 달라질 수 있으므로, 하드코딩된 절대경로 대신 `$HOME` 환경변수 기준으로 안내한다)에 받아 실행하는 것을 기본으로 한다. 실행 로그에서 `git push` 성공(원격 브랜치 ref 업데이트 라인 등)을 확인한 뒤에만, Claude는 사용자에게 다운로드했던 `.ps1` 파일 삭제를 안내한다(예: `Remove-Item "$HOME\Downloads\<파일명>.ps1" -Force`). push 성공을 확인하기 전에는 삭제를 권하지 않는다 — 실행이 중간에 실패했거나 재확인이 필요한 경우, 스크립트 파일 자체가 남아있어야 재시도/디버깅이 가능하기 때문이다.
...
- **[22차 세션 추가]** 세션 시작 후 Claude의 첫 응답에 "지침 문서 N차 버전 확인함"과 같은 언급이 없는 경우 — 4절 0번 단계(이 문서를 GitHub에서 가장 먼저 조회)를 건너뛰었을 가능성이 높다. 이 경우 이미 이 문서에 기록된 규칙(예: 21차의 .ps1 UTF-8 BOM 필수화)이 다시 무시된 채 스크립트가 만들어질 수 있으므로, 사용자는 스크립트를 실행하기 전에 Claude에게 0번 단계부터 다시 수행하도록 요청한다.
...
- **[22차 세션 추가]** 세션 시작 시 4절 0번 단계(이 지침 문서를 GitHub에서 최우선 조회)를 건너뛰고, 시스템 프롬프트에 고정 저장된 구버전 지침이나 이전 채팅에 붙여넣어진 오래된 사본만으로 작업을 시작하는 것 — 이미 이 문서에 반영되어 있는 수정 사항(예: 21차 BOM 규칙)을 모른 채 같은 실패를 반복하게 된다(22차에서 실제로 BOM 규칙이 무시된 채 재현됨, 4절·16절 참고).
...
- [22차 세션] (문서 변경) 22차 세션에서 프론트엔드 Google Drive 연결 UI(carrot-ryu, `web-gdrive-connect` 컴포넌트)를 반영하는 과정에서, 21차에서 이미 문서화했던 ".ps1 UTF-8 BOM 필수" 규칙을 그 세션의 Claude가 다시 위반해 동일한 콘솔/커밋 인코딩 손상이 재현됨(단, 이번에는 patch가 `git apply --check` 단계에서 실패해 커밋 전에 안전하게 중단됨 — 15절·18절의 "실패 시 강제 진행 금지" 원칙이 정상 작동한 사례). 원인 조사 결과, 그 세션이 4절 0번 단계(이 지침 문서를 GitHub에서 최우선 조회)를 수행하지 않고 시스템 프롬프트에 고정 저장된 9차 시점의 구버전 지침으로 시작한 것이 근본 원인으로 확인됨 — 즉 "같은 버그가 매 세션 반복된다"는 사용자 지적의 실제 원인은 규칙 누락이 아니라 규칙이 적힌 이 문서 자체를 조회하지 않고 세션이 시작되는 절차 누락이었음. 이에 따라 규칙을 새로 추가하는 대신, 기존 규칙(21차 BOM)은 그대로 유지하고 4절 0번 단계에 "조회 결과(확인한 최신 회차 번호)를 세션 첫 응답에서 사용자에게 보고" 의무를 추가해 사용자가 0번 단계 수행 여부를 즉시 검증할 수 있게 함. 16절에 이 보고가 없을 때 사용자가 취할 조치, 18절에 0번 단계 누락 자체를 금지 항목으로 추가. 4, 16, 18절 및 변경이력을 이에 맞춰 정리.
- [26차 세션] (문서 변경) 9절 끝에 "스크립트 실행 후 뒷정리" 규칙 추가. 사유: (1) 실행 명령 예시에 PC마다 다른 사용자명이 하드코딩된 절대경로(`C:\Users\<사용자명>\Downloads`)가 쓰이면 재사용성이 떨어지므로, `$HOME` 환경변수 기준 경로(`$HOME\Downloads\`)로 안내하도록 변경. (2) 실행 후 다운로드 폴더에 `.ps1` 파일이 계속 쌓이는 문제를 정리하되, `git push` 성공을 로그로 확인하기 전에는 삭제를 권하지 않아 실행 실패/재시도 상황에서 스크립트 파일이 남아있도록 함. 9절 및 변경이력을 이에 맞춰 정리.
- [26차 세션 추가2] (문서 변경) 9절 "매번 함께 전달" 1번 항목에 PowerShell 실행 정책 우회 안내 추가. 사유: 기본 실행 정책(`Restricted`)에서 `.ps1` 실행이 `UnauthorizedAccess` 오류로 막히는 문제가 24차와 26차에서 반복 발생했으나 24차 때 devnotes에 기록되지 않아 26차에서 사용자가 다시 처음부터 같은 오류를 겪음. 앞으로는 실행 명령을 `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass` + 실행 명령의 2줄 형태로 기본 안내하도록 정리(`-Scope Process`로 해당 PowerShell 창에만 적용되고 창을 닫으면 자동 복귀 — 시스템 전체 정책은 변경하지 않음). 9절 및 변경이력을 이에 맞춰 정리.
- [27차 세션] (문서 변경) 두 가지. (1) 9절 2번(git clone 단계)에 `--config core.autocrlf=false` 기본 포함 + Replace-Block 매칭 시 CRLF→LF 정규화 이중 방어 추가. 사유: 27차 세션에서 hud_renderer.py 수정 중 Windows Git의 core.autocrlf로 인해 LF 기준 anchor 블록이 실제 CRLF 파일 내용과 달라 0회 매치로 실패하는 사례가 실제로 재현됨(commit/push 이전 단계에서 안전하게 중단되어 데이터 손상은 없었음). `core.autocrlf=false` clone 옵션으로 재실행해 정상 해결을 확인, HANDOFF.md "다음 작업 후보 1번"으로 남겨뒀던 것을 사용자 승인으로 정식 반영. (2) 9절의 PowerShell 실행 정책 우회 명령에 `-Force` 옵션 추가. 사유: `-Scope Process`만으로는 "실행 정책을 변경하시겠습니까?" Y/A/N/L/S 확인 프롬프트가 여전히 뜨는 것이 확인되어(26차 세션 추가2 반영 이후에도 재현), `-Force`로 프롬프트 자체를 억제하도록 보완. 9절 및 변경이력을 이에 맞춰 정리.